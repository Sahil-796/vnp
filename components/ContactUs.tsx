...
export const ContactUs = () => {
  ...
  const source = searchParams.get("source") || "contact-page-direct";
  ...
  const meetingContext = packageName
    ? `Package: ${packageName}`
    : serviceSlug
      ? `Service: ${serviceSlug}`
      : "No package/service preselected";
  ...
  const prefill = {
    customAnswers: {
      ...
      a2: `Intent: ${intent}`,
      a3: meetingContext,
      a4: "Booked via website Calendly widget",
    },
  };
  ...
  const utm = {
    ...
    utmContent: intent,
    utmTerm: packageName || serviceSlug || "direct-contact",
  };
