...
const GlobalSpotlight: React.FC<{
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const cards = gridRef.current.querySelectorAll(BENTO_CARD_SELECTOR);
    ...
    const handleMouseLeave = () => {
      ...
      gridRef.current?.querySelectorAll(BENTO_CARD_SELECTOR).forEach((card) => {
        (card as HTMLElement).style.setProperty("--glow-intensity", "0");
      });
...
const MagicBento: React.FC<BentoProps> = ({
  children,
  className = "",
  gridClassName = "card-responsive grid gap-2",
  textAutoHide = true,
  enableStars = true,
